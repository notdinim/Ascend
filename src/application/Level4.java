...
public class Level4 extends Levels{
	...
	protected static MediaPlayer level4Music;
	
	// CONSTRUCTOR
	public Level4(Stage primaryStage, Image bg)
	{
		// PLAYING LEVEL 4 MUSIC
		Town.townMusicPlayer.pause();
		File bgMusic = new File("sounds\\level4.mp3");
		Media media = new Media(bgMusic.toURI().toString());
		level4Music = new MediaPlayer(media);
		level4Music.setOnEndOfMedia(new Runnable()
		{
			public void run() {
				level4Music.seek(Duration.ZERO);
			}
		});
		level4Music.setCycleCount(MediaPlayer.INDEFINITE);
		level4Music.play();
		...
		boss4 = new Boss4(545.7, 0);
		minion4 = new ArrayList<Boss4>();
		
		// ADDING LEVEL BACKGROUND AND BOSS TO SCREEN
		root.getChildren().addAll(iv, boss4.getNode(), boss4.redHP, boss4.greenHP);
