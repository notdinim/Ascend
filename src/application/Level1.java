...
public class Level1 extends Levels{
	...
	protected static MediaPlayer level1Music;
	
	// CONSTRUCTOR
	public Level1(Stage primaryStage, Image bg)
	{
		// PLAYING LEVEL 1 MUSIC
		Town.townMusicPlayer.pause();
		File bgMusic = new File("sounds\\level1.mp3");
		Media media = new Media(bgMusic.toURI().toString());
		level1Music = new MediaPlayer(media);
		level1Music.setOnEndOfMedia(new Runnable()
		{
			public void run() {
				level1Music.seek(Duration.ZERO);
			}
		});
		level1Music.setCycleCount(MediaPlayer.INDEFINITE);
		level1Music.play();
		...
		boss1 = new Boss1(545.7, 0);
		minion1 = new ArrayList<Boss1>();
		
		// ADDING LEVEL BACKGROUND AND BOSS TO SCREEN
		root.getChildren().addAll(iv, boss1.getNode(), boss1.redHP, boss1.greenHP);
