...
public class Level2 extends Levels{
	...
	protected static MediaPlayer level2Music;
	
	// CONSTRUCTOR
	public Level2(Stage primaryStage, Image bg)
	{
		// PLAYING LEVEL 2 MUSIC
		Town.townMusicPlayer.pause();
		File bgMusic = new File("sounds\\level2.mp3");
		Media media = new Media(bgMusic.toURI().toString());
		level2Music = new MediaPlayer(media);
		level2Music.setOnEndOfMedia(new Runnable()
		{
			public void run() {
				level2Music.seek(Duration.ZERO);
			}
		});
		level2Music.setCycleCount(MediaPlayer.INDEFINITE);
		level2Music.play();
		...
		boss2 = new Boss2(545.7, 0);
		minion2 = new ArrayList<Boss2>();
		
		// ADDING LEVEL BACKGROUND AND BOSS TO SCREEN
		root.getChildren().addAll(iv, boss2.getNode(), boss2.redHP, boss2.greenHP);
